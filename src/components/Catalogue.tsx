...
const FastFoodCatalogue: React.FC = () => {
  ...
  const handleAddToCart = () => {
    ...
    const newItem: CartItem = {
      ...
      serves: selectedProduct.quantity, // copy DB quantity
    };
  ...
  const handleProceedToBuy = () => {
    ...
    const message = `*New Fast Food Order*\n\n*Order ID:* ${newOrderId}\n\n${orderDetails}\n\n*Total: ₹${totalAmount.toFixed(
      2
            ...
            {currentProducts.map((product) => {
              ...
              return (
                <Card
                  ...
                  className="overflow-hidden rounded-2xl bg-white shadow-md hover:shadow-xl transition-all cursor-pointer flex flex-col border relative"
                >
                  ...
                  >
                    <div className="flex items-center gap-3 flex-1">
                      <div className="w-14 h-14 rounded-md overflow-hidden bg-gray-100 relative">
                        ...
                        />
                        <div
                          className={`absolute top-1 left-1 w-4 h-4 border rounded-sm flex items-center justify-center ${
                            item.isVeg
                              ? "border-green-600 bg-green-500"
                              : "border-red-600 bg-red-500"
                          }`}
                    ...
                    <div className="flex items-center gap-2">
                      <Button
                        ...
                        size="icon"
                        className="h-8 w-8"
                        onClick={() => decreaseQty(item.id, item.portion)}
                      >
                        <Minus size={12} />
                      </Button>
                      ...
                      <Button
                        ...
                        size="icon"
                        className="h-8 w-8"
                        onClick={() => increaseQty(item.id, item.portion)}
                      >
                        <Plus size={12} />
                      </Button>
                      <Button
